...
class ClientRecord {
  constructor(obj) {
    const {
      id, name, mail, nextContactAt, notes,
    } = obj;
    ...
    if (!mail || typeof mail !== 'string' || mail.indexOf('@') == -1) {
      throw new ValidationError('E-mail nieprawidlowy.');
    }

    if (typeof nextContactAt !== 'string') {
      throw new ValidationError('Data nastepnego kontaktu musi byc tekstem');
    }

    if (typeof notes !== 'string') {
      throw new ValidationError('Notatki  muszą byc tekstem');
    }
